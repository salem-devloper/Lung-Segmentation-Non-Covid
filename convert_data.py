...
def create_annotation(path):
    ...
    covid_images =[image for image in images if image+'_mask' in masks]
    no_covid_images =[image for image in images if image+'_mask' not in masks]
...
def create_original_data(path,out):
    ...
    for img_file in tqdm(covid_images):
        ...
        copyfile(os.path.join(masks_path,img_file+'_mask'),
                os.path.join(masks_out,img_file+'_mask'))

        img = np.array(Image.open(os.path.join(images_path,img_file)).convert('L'))

        mask = np.array(Image.open(os.path.join(masks_path,img_file+'_mask')).convert('L'))
